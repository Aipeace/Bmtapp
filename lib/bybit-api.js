 ...
 *  9. Bybit P2P returns ret_code/ret_msg (snake_case) — documented in comments
 */
...
class BybitP2PApi {
  ...
  async _get(path, params = {}) {
    ...
    // FIX #9: surface Bybit P2P application-level errors (ret_code != 0)
    if (json.ret_code !== undefined && json.ret_code !== 0) {
      throw new Error(`Bybit P2P error ${json.ret_code}: ${json.ret_msg}`);
    }
    return json;
  }
   ...
   * Was: GET /v5/account/wallet-balance (spot/futures, not P2P fund)
   * Now: GET /v5/asset/transfer/query-account-coins-balance
   */
