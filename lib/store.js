 ...
 *   apiSecret    : string,      // Bybit API secret (AES-256-CBC encrypted at rest)
 *   testnet      : boolean,
 *   active       : boolean,     // admin can suspend
 *   isAdmin      : boolean,
 *   watchOrders  : boolean,     // persisted /watch subscription
 *   priceAlerts  : Array,       // persisted price alert list
...
// ── Encryption helpers ────────────────────────────────────────────────────────
// ENC_KEY is read lazily inside encrypt/decrypt so it always picks up the
// fully-loaded process.env (important when dotenv is called after require).
function getEncKey() {
  return (process.env.APP_SECRET || 'change_me_32_chars_exactly!!!!!!').slice(0, 32);
}
const IV_LEN = 16;
...
function encrypt(text) {
  ...
  const c  = crypto.createCipheriv('aes-256-cbc', Buffer.from(getEncKey()), iv);
...
function decrypt(text) {
  ...
  const d = crypto.createDecipheriv('aes-256-cbc', Buffer.from(getEncKey()), Buffer.from(ivHex, 'hex'));
