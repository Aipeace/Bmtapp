...
bot.on('callback_query', async (q) => {
  ...
  // Ad: toggle  — data format: ad_toggle_<adId>_<status>
  // Ad IDs may contain underscores so we parse from the right
  if (data.startsWith('ad_toggle_')) {
    const withoutPrefix = data.slice('ad_toggle_'.length);  // "<adId>_<status>"
    const lastUnderscore = withoutPrefix.lastIndexOf('_');
    const adId      = withoutPrefix.slice(0, lastUnderscore);
    const curStatus = withoutPrefix.slice(lastUnderscore + 1);
